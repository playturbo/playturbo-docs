...
3）为血条图层添加动画-通用-缩放缓动，重要参数设置：单项循环/起始比例不变，结束比例Y轴不变，都默认为1，仅调整结束比例X轴的数值即可（此处的数值代表最终血量）。具体参数设置如下：
